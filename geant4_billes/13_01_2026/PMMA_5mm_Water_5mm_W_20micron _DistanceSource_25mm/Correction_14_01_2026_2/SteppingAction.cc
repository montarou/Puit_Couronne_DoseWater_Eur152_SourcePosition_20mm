...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    // Entrée dans le container (gamma primaire) - avec protection anti-double-comptage
    ...
    // ═══════════════════════════════════════════════════════════════
    // ENTRÉE DANS L'EAU - CORRIGÉ : 
    // - Seulement primaires
    // - Pas de double-comptage (HasEnteredWater)
    // - Ignorer les transitions depuis les plans de comptage
    // ═══════════════════════════════════════════════════════════════
    
    // Les plans de comptage ne sont pas des vrais volumes physiques,
    // ils ne doivent pas déclencher une "entrée dans l'eau"
    ...
    if (fWaterRingNames.find(postLogVolName) != fWaterRingNames.end() 
        ...
        && !fromCountingPlane) {  // NOUVEAU : ignorer transitions depuis plans de comptage
        ...
        if (particleName == "gamma" && parentID == 0) {
            ...
            if (!fEventAction->HasEnteredWater(trackID)) {
                ...
                // LOG déplacé ICI pour éviter les messages trompeurs
                if (fVerbose && eventID < fVerboseMaxEvents) {
                    G4ThreeVector posPost = postStepPoint->GetPosition();
