...
void RunAction::BeginOfRunAction(const G4Run* run)
{
    ...
    analysisManager->CreateH1("hElectronSpectrum", "Electrons secondaires;Energie (keV);Counts", 500, 0., 1000.);
    ...
    analysisManager->CreateH1("h_dose_ring0", "Dose Anneau 0 (0-5mm);Dose (nGy);Counts", 500, 0., 2.0);
    analysisManager->CreateH1("h_dose_ring1", "Dose Anneau 1 (5-10mm);Dose (nGy);Counts", 500, 0., 1.0);
    analysisManager->CreateH1("h_dose_ring2", "Dose Anneau 2 (10-15mm);Dose (nGy);Counts", 500, 0., 0.5);
    analysisManager->CreateH1("h_dose_ring3", "Dose Anneau 3 (15-20mm);Dose (nGy);Counts", 500, 0., 0.3);
    analysisManager->CreateH1("h_dose_ring4", "Dose Anneau 4 (20-25mm);Dose (nGy);Counts", 500, 0., 0.2);
    
    // H1 ID=15: Dose totale par événement
    analysisManager->CreateH1("h_dose_total", "Dose totale eau;Dose (nGy);Counts", 500, 0., 1.0);
    ...
    // MODIFIÉ : Ajout des branches pour les processus d'absorption
    // ═══════════════════════════════════════════════════════════════
    analysisManager->CreateNtuple("gamma_lines", "Statistiques par raie gamma");
    analysisManager->CreateNtupleIColumn("lineIndex");        // Col 0
    analysisManager->CreateNtupleDColumn("energy_keV");       // Col 1
    analysisManager->CreateNtupleIColumn("emitted");          // Col 2
    analysisManager->CreateNtupleIColumn("enteredWater");     // Col 3
    analysisManager->CreateNtupleIColumn("absorbedWater");    // Col 4
    analysisManager->CreateNtupleDColumn("waterAbsRate");     // Col 5
    analysisManager->CreateNtupleDColumn("waterEntryRate");   // Col 6
    // ═══════════════════════════════════════════════════════════════
    // NOUVELLES BRANCHES : Processus d'absorption
    // ═══════════════════════════════════════════════════════════════
    analysisManager->CreateNtupleIColumn("nPhotoelectric");   // Col 7
    analysisManager->CreateNtupleIColumn("nCompton");         // Col 8
    analysisManager->CreateNtupleIColumn("nPairProduction");  // Col 9
    analysisManager->CreateNtupleIColumn("nOther");           // Col 10
    analysisManager->FinishNtuple();
...
void RunAction::EndOfRunAction(const G4Run* run)
{
    ...
    // MODIFIÉ : Ajout des données de processus d'absorption
    // ═══════════════════════════════════════════════════════════════
    ...
    for (G4int i = 0; i < EventAction::kNbGammaLines; ++i) {
        ...
        // Colonnes existantes
        analysisManager->FillNtupleIColumn(3, 0, i);                           // lineIndex
        analysisManager->FillNtupleDColumn(3, 1, energy_keV);                  // energy_keV
        analysisManager->FillNtupleIColumn(3, 2, fLineEmitted[i]);             // emitted
        analysisManager->FillNtupleIColumn(3, 3, fLineEnteredWater[i]);        // enteredWater
        analysisManager->FillNtupleIColumn(3, 4, fLineAbsorbedWater[i]);       // absorbedWater
        analysisManager->FillNtupleDColumn(3, 5, waterAbsRate);                // waterAbsRate
        analysisManager->FillNtupleDColumn(3, 6, waterEntryRate);              // waterEntryRate
        
        // ═══════════════════════════════════════════════════════════════
        // NOUVELLES COLONNES : Processus d'absorption
        // ═══════════════════════════════════════════════════════════════
        analysisManager->FillNtupleIColumn(3, 7, fLineAbsorbedByProcess[i][EventAction::kPhotoelectric]);  // nPhotoelectric
        analysisManager->FillNtupleIColumn(3, 8, fLineAbsorbedByProcess[i][EventAction::kCompton]);        // nCompton
        analysisManager->FillNtupleIColumn(3, 9, fLineAbsorbedByProcess[i][EventAction::kPairProduction]); // nPairProduction
        analysisManager->FillNtupleIColumn(3, 10, fLineAbsorbedByProcess[i][EventAction::kOther]);         // nOther
    ...
    oss << "║  Gammas entrant container   : " << std::setw(12) << fGammasEnteringContainer << "                                    ║\n";
    oss << "║  Gammas entrant eau         : " << std::setw(12) << fGammasEnteringWater << "                                    ║\n";
    ...
    // ═══════════════════════════════════════════════════════════════
    // NOUVEAU : TABLEAU DES PROCESSUS D'ABSORPTION PAR RAIE
    // ═══════════════════════════════════════════════════════════════
    
    oss << "\n╔═══════════════════════════════════════════════════════════════════════════════════════════════════════════════╗\n";
    oss << "║                              PROCESSUS D'ABSORPTION DANS L'EAU                                                ║\n";
    oss << "╠═══════════════╦═══════════════╦═══════════════════╦═══════════════════╦═══════════════════╦═══════════════════╣\n";
    oss << "║     Raie      ║   Abs. eau    ║   Photoélectrique ║      Compton      ║  Création paires  ║       Autres      ║\n";
    oss << "╠═══════════════╬═══════════════╬═══════════════════╬═══════════════════╬═══════════════════╬═══════════════════╣\n";
    
    for (G4int i = 0; i < EventAction::kNbGammaLines; ++i) {
        G4int total = fLineAbsorbedWater[i];
        G4double photPct = (total > 0) ? 100.0 * fLineAbsorbedByProcess[i][EventAction::kPhotoelectric] / total : 0;
        G4double comptPct = (total > 0) ? 100.0 * fLineAbsorbedByProcess[i][EventAction::kCompton] / total : 0;
        G4double convPct = (total > 0) ? 100.0 * fLineAbsorbedByProcess[i][EventAction::kPairProduction] / total : 0;
        G4double otherPct = (total > 0) ? 100.0 * fLineAbsorbedByProcess[i][EventAction::kOther] / total : 0;
        
        oss << "║ " << std::setw(13) << std::left << EventAction::GetGammaLineName(i) << " ║"
            << std::setw(14) << std::right << total << " ║"
            << std::setw(8) << fLineAbsorbedByProcess[i][EventAction::kPhotoelectric] << " (" << std::setw(5) << std::fixed << std::setprecision(1) << photPct << "%) ║"
            << std::setw(8) << fLineAbsorbedByProcess[i][EventAction::kCompton] << " (" << std::setw(5) << comptPct << "%) ║"
            << std::setw(8) << fLineAbsorbedByProcess[i][EventAction::kPairProduction] << " (" << std::setw(5) << convPct << "%) ║"
            << std::setw(8) << fLineAbsorbedByProcess[i][EventAction::kOther] << " (" << std::setw(5) << otherPct << "%) ║\n";
    }
    oss << "╚═══════════════╩═══════════════╩═══════════════════╩═══════════════════╩═══════════════════╩═══════════════════╝\n";
