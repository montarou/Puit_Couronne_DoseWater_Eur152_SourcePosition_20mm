...
void analyse_absorption_raies(const char* filename = "output.root")
{
    ...
    h_absRate->SetMinimum(0.001);  // Minimum > 0 pour echelle log
    ...
    g_absVsE->SetMinimum(0.001);  // Minimum pour echelle log
    g_absVsE->SetMaximum(10);     // Maximum pour echelle log
    
    g_absVsE->Draw("APL");
    
    // Annotations pour chaque point (adapté pour échelle log)
    TLatex latex2;
    latex2.SetTextSize(0.025);
    for (Int_t i = 0; i < nEntries; i++) {
        double ypos = (absRates[i] > 0.01) ? absRates[i] * 1.5 : 0.015;
        latex2.DrawLatex(energies[i]*1.05, ypos, Form("%.2f%%", absRates[i]));
    }
