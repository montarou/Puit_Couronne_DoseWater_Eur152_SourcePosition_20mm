...
void analyse_Dose(const char* filename = "output.root")
{
    ...
    gStyle->SetOptStat(0);        // 10 = seulement Entries
    ...
    if (hGammaEmitted && hGammaEmitted->GetEntries() > 0) {
        ...
        hGammaEmitted->Rebin(5);

        hGammaEmitted->SetLineColor(kRed+2);
        hGammaEmitted->SetFillColor(kRed);      // Couleur de remplissage
        hGammaEmitted->SetFillStyle(1001);      // Remplissage plein
        ...
        hGammaEmitted->Draw("HIST");            // Option HIST pour bins coloré
    ...
    if (hGammaWater && hGammaWater->GetEntries() > 0) {
        ...
        hGammaWater->SetMarkerStyle(20);    // Cercle plein
        hGammaWater->SetMarkerSize(0.8);    // Taille des symboles augmentée
        ...
        hGammaWater->Draw("P");             // Option P pour afficher les marqueurs
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (h_dose_ring[i] && h_dose_ring[i]->GetEntries() > 0) {
            ...
            // ═══════════════════════════════════════════════════════════════
            // MODIFICATION: Symboles carrés plus petits
            // ═══════════════════════════════════════════════════════════════
            h_dose_ring[i]->SetLineColor(ringColors[i]);
            h_dose_ring[i]->SetMarkerColor(ringColors[i]);
            h_dose_ring[i]->SetMarkerStyle(21);  // Carré plein (était 20 cercle)
            h_dose_ring[i]->SetMarkerSize(0.6);  // Taille réduite
    ...
    if (gamma_lines) {
        ...
        h_abs_water->SetMinimum(0.0001);   // Minimum > 0 pour echelle log
        ...
        latex->SetTextSize(0.04);
        latex->DrawLatex(0.15, 0.87, "Source Eu-152 (42 kBq) - Eau 5 mm - SANS FILTRE");
        latex->DrawLatex(0.15, 0.83, Form("Absorption max: %.1f%% (raies X 40 keV)", maxRate));
    ...
    gStyle->SetOptStat(0);  // Afficher seulement Entries
    ...
    if (hElectron && hElectron->GetEntries() > 0) {
        ...
        hElectron->SetMarkerStyle(20);    // Cercle plein
        hElectron->SetMarkerSize(0.6);    // Taille augmentée
        hElectron->SetTitle("Spectre des electrons secondaires dans l'eau");
        hElectron->Draw("P");             // Option P pour afficher les marqueurs
